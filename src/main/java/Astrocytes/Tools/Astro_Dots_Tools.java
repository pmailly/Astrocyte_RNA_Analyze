    ...
    */
    public static void find_background(ImagePlus imgAstro) {
        ...
        for (int n = 1; n <= img1.getNSlices(); n++) {
            ImageStatistics stats = new ImageStatistics().getStatistics(img1.getStack().getProcessor(n), Measurements.MEAN, cal);
            meanInt += stats.mean;
            //System.out.println(meanInt);
        }
        bg = meanInt/img1.getNSlices();
        System.out.println("Background = "+bg);
        flush_close(img);
        flush_close(imgMask);
        flush_close(img1);
